...
const SimplifiedHackathonView = ({ analysis }) => {
  const { decision, features, inference } = analysis;
  const score = decision.readinessScore;
  const [detailsOpen, setDetailsOpen] = useState(false);

  // Decision Logic
  let state = "READY TO EAT";
  let colorClass = "text-green-500";
  let explanation = "Your body has finished recovery. Eating now is safe.";
  let scoreColor = "text-green-400";
  let bgPulse = "bg-green-500/5";
  let border = "border-green-500/30";

  if (score < 40) {
    state = "DO NOT EAT YET";
    colorClass = "text-red-500";
    explanation = "Eating now may cause metabolic stress.";
    ...
    explanation = "Your metabolism is still waking up.";
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-20 space-y-8 animate-fade-in relative z-10 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
        ...
        <div className={`lg:col-span-12 xl:col-span-8 p-10 rounded-3xl border-2 ${border} ${bgPulse} relative overflow-hidden flex flex-col items-center justify-center text-center shadow-2xl backdrop-blur-sm transition-all duration-700 hover:scale-[1.01]`}>
...
const HomeView = ({ onRunPipeline, onManualMode, manualMode, onManualSubmit, inputData, setInputData, setManualMode, fileInputRef }) => (
  <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center max-w-5xl mx-auto">
    ...
    <p className="text-xl text-slate-400 mb-12">
      Select an input source to analyze metabolic readiness.
    </p>

    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
      {/* STANDARD CARD */}
      <div className="bg-slate-800/40 p-8 rounded-3xl border border-slate-700 flex flex-col gap-4 shadow-xl">
        <h2 className="text-xl font-bold text-white mb-2 flex items-center justify-center gap-2"><Upload size={20} /> Standard Analysis</h2>
        {!manualMode ? (
          <>
            <button onClick={() => onRunPipeline(null)} className="w-full bg-blue-600 hover:bg-blue-500 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-2">
              <Play size={20} /> Load Demo Data
            </button>
            <div className="flex gap-4">
              <button onClick={() => fileInputRef.current.click()} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-2">
                <Upload size={18} /> Upload
                <input type="file" ref={fileInputRef} className="hidden" accept=".csv,.json" onChange={e => { if (e.target.files[0]) onRunPipeline(e.target.files[0]) }} />
              </button>
              <button onClick={() => setManualMode(true)} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-2">
                <Edit3 size={18} /> Manual
              </button>
            </div>
          </>
        ) : (
          <div className="text-left bg-slate-900/50 p-4 rounded-xl border border-slate-700">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-bold text-white">Manual Entry</span>
              <button onClick={() => setManualMode(false)} className="text-xs text-slate-400">Cancel</button>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-2">
              <input type="time" className="bg-slate-800 border-slate-600 rounded p-2 text-sm" value={inputData.sleepTime} onChange={e => setInputData({ ...inputData, sleepTime: e.target.value })} />
              <input type="time" className="bg-slate-800 border-slate-600 rounded p-2 text-sm" value={inputData.wakeTime} onChange={e => setInputData({ ...inputData, wakeTime: e.target.value })} />
            </div>
            <button onClick={onManualSubmit} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-lg font-bold text-sm">Analyze</button>
          </div>
        )}
      </div>

      {/* SIMULATION CARD */}
      <div className="bg-slate-900/80 p-8 rounded-3xl border border-slate-700/80 flex flex-col gap-4 shadow-xl relative overflow-hidden group">
        <div className="absolute inset-0 bg-emerald-500/5 group-hover:bg-emerald-500/10 transition-colors"></div>
        <div className="relative z-10 flex flex-col h-full justify-between">
          <div>
            <h2 className="text-xl font-bold text-emerald-400 mb-2 flex items-center justify-center gap-2"><Zap size={20} /> Hardware Simulation</h2>
            <p className="text-sm text-slate-400 leading-relaxed mb-6">
              Launch a virtual wearable environment with <strong>Live Streaming Data</strong>.
            </p>
          </div>
          <button onClick={() => onRunPipeline({ mode: 'simulation' })} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white p-5 rounded-2xl transition-all font-bold flex items-center justify-center gap-3">
            <Activity size={24} className="animate-pulse" /> Launch Live Simulator
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto flex flex-col">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-xs font-mono uppercase ${isSim ? 'text-emerald-500' : 'text-blue-400'}`}>
              {isSim ? "System Active • Generating Stream" : "Static Analysis • Complete"}
            </span>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 flex-1">
        <div className="lg:col-span-1 space-y-6">
          <div className={`p-8 rounded-2xl border-2 ${scoreColor} text-center shadow-lg relative overflow-hidden backdrop-blur-sm transition-all duration-500`}>
        ...
        <div className="lg:col-span-2 flex flex-col">
          <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 h-[500px] flex flex-col shadow-xl">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-lg text-white">Circadian Timeline</h3>
              <div className="flex gap-2 text-xs">
                <span className="text-indigo-400">● Sleep</span>
                <span className="text-emerald-400">-- Wake</span>
                {isSim && <span className="text-emerald-500 animate-pulse">● LIVE</span>}
              </div>
            </div>
            <div className="flex-1 w-full min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.data}>
                  ...
                  <ReferenceArea x1={features.output.sleep_start} x2={features.output.wake_time} fill="#4f46e5" fillOpacity={0.05} />
                  <ReferenceArea
                    x1={optimalEatingTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
                    x2={new Date(optimalEatingTime.getTime() + 60 * 60000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
      ...
      {/* --- NEW SECTION: Enhanced Hackathon UI --- */}
      <SimplifiedHackathonView analysis={analysis} />
    </div>
  );
};
...
const App = () => {
  ...
  const [inputData, setInputData] = useState({ sleepTime: "23:00", wakeTime: "07:00", minHR: 50, avgHR: 75 });
  ...
  const runPipeline = async (source) => {
    ...
    setTimeout(async () => {
      ...
      updateAnalysis(dailyData);

      if (isSim) {
        setView('results_simulation');
        startLiveStream();
      } else {
        setView('results_static');
      }
    }, isSim ? 2000 : 1000);
  };

  const updateAnalysis = (dailyData) => {
    const features = FeatureExtractionLayer.extract(dailyData);
    const inference = InferenceEngine.analyze(features);
    const decision = DecisionEngine.decide(inference);
  ...
  const startLiveStream = () => {
    liveInterval.current = setInterval(() => {
      ...
      const newPoint = DataLayer.generateNextPoint(lastPoint);
      const newData = [...currentData.slice(1), newPoint];

      liveDataRef.current = newData;
      setData({ data: newData }); // State update triggers render

      if (newData.length % 10 === 0) {
        updateAnalysis({ data: newData });
      }
    }, 200);
  };
  ...
  return (
    <div className="bg-slate-900 min-h-screen text-slate-100 font-sans selection:bg-blue-500/30">
      {view === 'home' && <HomeView
        ...
        onManualMode={() => setManualMode(!manualMode)}
        manualMode={manualMode}
        onManualSubmit={(e) => { e.preventDefault(); runPipeline({ mode: 'manual', data: inputData }) }}
        inputData={inputData}
      ...
      {(view === 'results_static' || view === 'results_simulation') && analysis && (
        <ResultsDashboard
          analysis={analysis}
          data={data}
          mode={view === 'results_simulation' ? 'simulation' : 'static'}
